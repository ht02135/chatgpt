//-----------------------------------
// customize-components-2.0.js (Refactored for action-driven model)
//-----------------------------------
...
//customize-form-actions (renamed and unified)
    ...
    <div class="container">
      ...
      <div>
        ...
        <customize-form-actions 
          params="actions: $component.actions,
...
//customize-search-actions (kept for search screens)
...
// Make sure API_BATCH_DOWNLOADS_REQUEST is imported from constants.js
    ...
    <table>
      ...
      <tbody data-bind="foreach: $component.items">
        <tr data-bind="foreach: $parent.gridConfig.columns.filter(c => c.visible)">
          ...
          <!-- ko if: name !== 'actions' -->
          <td>
            <!-- ko if: type === 'jobRequestDownload' -->
              <a data-bind="
                attr: { 
                  href: window.API_BATCH_DOWNLOADS_REQUEST + '?jobRequestId=' + ($parent.id ? $parent.id() : ''),
                  target: '_blank'
                },
                text: $parent[$data.name] ? ($parent[$data.name]()) : ''
              "></a>
            <!-- /ko -->

            <!-- ko if: type !== 'jobRequestDownload' -->
              <span data-bind="text: $parent[$data.name] ? $parent[$data.name]() : ''"></span>
            <!-- /ko -->
          </td>
          <!-- /ko -->
