...
public class Step3PopulateUserListChunkByInnerClass extends AbstractJobRequestByDelegateStep {
	...
	public Step step3PopulateUserListByInnerClass(StepBuilderFactory stepBuilderFactory) {
		...
		return stepBuilderFactory.get("step3PopulateUserListByInnerClass")
				.<UserManagementPojo, UserManagementListMemberPojo>chunk(50).reader(new UserReader())
				.processor(new UserProcessor()).writer(new UserWriter()).listener(this).build();
	}
	...
	private class UserReader extends MyBatisCursorItemReader<UserManagementPojo> {
		...
		public UserManagementPojo read() throws Exception {
			jobRequest = getOneRecentJobRequestByParams(UserListJobConfig.JOB_NAME, 300, 1,
